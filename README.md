# Вебзастосунок для кіноманів 🎬

Це Single Page Application (SPA), що виконує роль каталогу фільмів. Застосунок дозволяє користувачам переглядати список фільмів, фільтрувати їх, читати детальний опис та залишати коментарі, які зберігаються локально.
...
- `components/` — перевикористовувані UI-елементи (наприклад, `MovieCard`, `Layout`, `CommentsSection`).
- `pages/` — сторінки додатку (`Home`, `Catalog`, `MovieDetails`, `Blog`, `Contacts`).
...
- `types/` — TypeScript інтерфейси (наприклад, тип `Movie`).
...
> **ВАЖЛИВО!** Перед деплоєм обов'язково відкрийте файл `vite.config.ts` та замініть значення `base: '/назва-репозиторію/'` на реальну назву вашого репозиторію на GitHub (наприклад, `base: '/media-library/'`).
